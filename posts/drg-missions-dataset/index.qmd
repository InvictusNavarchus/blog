...
missions_df_combined <- read_csv('csv/drg_missions_july_2024.csv')
```
...
Let's start from daily, since it's the simplest. All we need to do is to summarize the data from each day and plot them on a graph. We'd also need a separate column for the date.

```{r}
missions_df_combined_cleaned |> 
  mutate(date = days_in_month(Timestamp),
         .after = Timestamp)
```
